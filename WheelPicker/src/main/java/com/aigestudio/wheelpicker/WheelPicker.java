...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
     ...
     * The number of data items visible in the wheel selector and the number of data items the wheel selector will draw
     *
     ...
     * The minimum/maximum Y coordinate that the slider can slide to when it slides
     */
    private int mMinFlingY, mMaxFlingY;

    /**
     * 滚轮滑动时的最小/最大速度
     * Minimum/maximum speed when the roller is sliding
     */
    ...
    /**
     * 点击与触摸的切换阀值
     * Click and touch toggle threshold
     */
    ...
    public void setForceFinishScroll(boolean forceFinishScroll) {
        isForceFinishScroll = forceFinishScroll;
    }
    ...
    public void run() {
        ...
        if (mScroller.isFinished() && !isForceFinishScroll) {
            ...
            if (null != mOnItemSelectedListener && isTouchTriggered) {
                ...
                if(mListenerData != null) o = mListenerData.get(position);
                mOnItemSelectedListener.onItemSelected(this, o, position);
            }
    ...
    public void setListenerData(List mListenerData) {
        this.mListenerData = mListenerData;
        if(this.mData == null)
            this.mData = mListenerData;
    }
